...
export const sortObject = <T extends arrayObject>(arr:T,target:string,sort:sortConfig):T => {
    ...
    if(targetArr instanceof Array){
        ...
        if(targetArr.length < 1){
            ...
            //判断数组中每一项是否对象
            let isObj:boolean = targetArr.every(v => v instanceof Object);
            //判断传入的属性是否在数组的每个对象中，且为可排序类型
            let isRealKey:boolean = targetArr.every( v => {
                let keysArr:Array<string> = Object.keys(v);
                return keysArr.includes(target)
            });
            if(isObj&&isRealKey){
                let len:number = targetArr.length;
